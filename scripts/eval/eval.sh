...
usage() {
    cat >&2 <<'USAGE'
Usage:
  scripts/eval/eval.sh [--dry-run] path_to_config
...
export REPO_ROOT TEMPLATE_ROOT EVAL_LIB_DIR CONFIG_PATH DRY_RUN

# shellcheck disable=SC1091
source "$EVAL_LIB_DIR/runtime.sh"
# shellcheck disable=SC1091
source "$EVAL_LIB_DIR/commands.sh"
# shellcheck disable=SC1091
source "$EVAL_LIB_DIR/vllm.sh"
# shellcheck disable=SC1091
source "$EVAL_LIB_DIR/job_config.sh"
