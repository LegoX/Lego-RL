...
`TOOL_CALL_PARSER` (default `qwen3_coder`), which drives every config point
(`lego_rl_*.yaml` → `tool-call-parser`, `agent_loop_config_cc.yaml` →
`tool_parser`):

| Model output format | `TOOL_CALL_PARSER` |
| --- | --- |
...
— see [Core Concepts](/docs/getting-started#1-venv).
</Callout>
